...
exports.getStores = async (req, res) => {
  ...
  if (!stores.length && skip) {
    req.flash('info', `Hey you asked for page ${page} but it does not exist! So we put you on ${pages}`)
...
exports.mapStores = async (req, res) => {
  const coordinates = [req.query.lng, req.query.lat].map(x => parseFloat(x));
  const q = {
    location: {
      $near: {
        $geometry: {
